...
int_t2:
    BCF ((PIR1) and 07Fh), 1 ; reiniciamos la bandera de interrupción de TIMER2
    BTFSC PORTB, 0
    goto $+3
    BSF PORTB, 0
    goto $+4
...
config_clock:
    ...
    BSF ((OSCCON) and 07Fh), 4 ; IRCF <2:0> -> 011 500 kHz
...
config_tmr0:
    ...
    BCF ((OPTION_REG) and 07Fh), 0 ; PS<2:0> -> preescaler 000 1:1
...
config_tmr1:
    ...
    BSF ((T1CON) and 07Fh), 4 ; T1CKPS <1:0> -> 01 Prescaler 1:2

    BCF ((T1CON) and 07Fh), 3 ;reloj interno
    BCF ((T1CON) and 07Fh), 1
    BSF ((T1CON) and 07Fh), 0 ;prender TMR1
...
config_int_enable:
    banksel TRISA
    BSF ((PIE1) and 07Fh), 0 ; Habilitar interrupción del TIMER1
    BSF ((PIE1) and 07Fh), 1 ; Habilitar interrupción del TIMER2

    banksel PORTA
    BSF ((INTCON) and 07Fh), 7 ; INTCON: Habilitar interrupciones globales
    BSF ((INTCON) and 07Fh), 6 ; Habilitar interrupciones de periféricos

    BCF ((PIR1) and 07Fh), 0 ; Limpiar bandera de interrupción de TIMER1
    BCF ((PIR1) and 07Fh), 1 ; Limpiar bandera de interrupción de TIMER2

    BSF ((INTCON) and 07Fh), 5 ; Habilitar interrupción del TIMER0
    BCF ((INTCON) and 07Fh), 2 ; Limpiar bandera de interrupciones
    return
